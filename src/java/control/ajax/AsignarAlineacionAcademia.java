...
public class AsignarAlineacionAcademia extends ActionSupport {
            ...
            for (int i = 0; i < ids.length; i++) {
                for (int j = 0; j < ids.length; j++) {
                    if (i != j && ids[i] > 0 && posiciones[i] > 0 && posiciones[i] < 16 && (ids[i] == ids[j] || posiciones[i] == posiciones[j])) {
                        response = "Lo sentimos, se ha detectado un error en esa alineación";
                        return SUCCESS;
                    }
                }
            }
            usuarioDAO.setAlineacionAcademia(userId, alineacion);
            session.close();
            response = "";
            return SUCCESS;

        } catch (Exception e) {
            response = "Lo sentimos, se ha producido un error con esa alineación";
            return SUCCESS;
        }
    }
